...
"""
Heavily inspired (copied) from airflow BigQuery integration
https://github.com/apache/incubator-airflow/blob/master/airflow/contrib/hooks/bigquery_hook.py

Which in turn attributes the hive implementation of PEP 249
...
from ..authorize.authorize import GoogleCloudBaseHook
...
class BigQueryConnection(GoogleCloudBaseHook):
    """
    ...
    def cursor(self):
        ...
        return BigQueryCursor(service, self._args[0])

    def rollback(self):
        raise NotImplementedError(
            "BigQueryConnection does not have transactions")

    def get_table_names(self, schema, **kw):
        #iterate through dataset ids and table ids
...
class BigQueryCursor(BigQueryBaseCursor):
    ...
    def description(self):
        ...
        """This read-only attribute is a sequence of 7-item sequences.
                ...
                """
        if not self.job_id:
            logging.info('no job_id : %s', self.project_id)
            return -1
        ...
        if len(self.buffer) == 0:
            ...
            schema = query_results['schema']['fields']
            return [
    ...
    @property
    def rowcount(self):
        logging.info('rowcount, project_id : %s buffer: %s', self.project_id, self.buffer)
        if not self.job_id:
            logging.info('no job_id : %s', self.project_id)
            return -1

        if len(self.buffer) == 0:
            logging.info('self.buffer: %s', self.buffer)
            if self.all_pages_loaded:
                return None
        ...
        if 'rows' in query_results and query_results['totalRows']:
            return query_results['totalRows']
